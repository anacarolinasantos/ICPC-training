...
public class Graph {
	...
	public boolean isConnected() {
		boolean[] connected = new boolean[this.vertices.size()];

		Set<String> vertices = this.vertexKeys();
		Vertex current = this.vertices.get(vertices.iterator().next());

		while (current != null){
			connected[Integer.parseInt(current.getLabel())] = true;

			ArrayList<Edge> adjacencyList = current.getNeighbors();

			Edge edge = adjacencyList.get(0);

			if (connected[Integer.parseInt(edge.getOne().getLabel())] == false) {
				connected[Integer.parseInt(edge.getOne().getLabel())] = true;
				current = edge.getOne();
			} else if (connected[Integer.parseInt(edge.getTwo().getLabel())] == false) {
				connected[Integer.parseInt(edge.getTwo().getLabel())] = true;
				current = edge.getTwo();
			} else {
				if (adjacencyList.get(1).getTwo() == current) {
					current = adjacencyList.get(2).getTwo();
					if (current == null) {
						current = adjacencyList.get(2).getOne();
					}
				} else {
					current = adjacencyList.get(1).getTwo();
					if (current == null) {
						current = adjacencyList.get(1).getOne();
					}
				}
